    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (Element element : roundEnv.getElementsAnnotatedWith(ImmutableDataMapper.class)) {
            ...
            MethodSpec setter = MethodSpec.methodBuilder("set" + capitalizedFieldName)
                    .addModifiers(Modifier.PUBLIC)
                    .addParameter(element.getClass(), fieldName) // Adjust the parameter type based on the field type
                    .addStatement("this.$N = $N", fieldName, fieldName)
                    .build();

            // Create a new class with the generated methods
            TypeSpec accessorClass = TypeSpec.classBuilder(className + "Accessors")
                    .addModifiers(Modifier.PUBLIC)
                    .addMethod(getter)
                    .addMethod(setter)
                    .build();
            ...
            JavaFile javaFile = JavaFile.builder("com.skillbox.cryptobot.generated", accessorClass)
                    .build();
